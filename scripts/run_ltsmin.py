...
def main():
    ...
    parser.add_argument(
        "-t", "--ltsmin-binpath", action="store", type=str, required=True
    )
    parser.add_argument(
        "-n",
        "--runs",
    ...
    for run in range(1, args.runs + 1):
        for alg in ["branching-bisim"]:
            ...
            for file in os.listdir(os.path.join(SCRIPT_PATH, "lts")):
                if ".aut" in file:
                    ...
                    print(run_result)
                    results[file] = run_result

            # Add run number suffix to the result file name
            with open(
                os.path.join("results", f"ltsmin_{alg}_{run}.json"),
                "w",
                encoding="utf-8",
            ) as json_file:
